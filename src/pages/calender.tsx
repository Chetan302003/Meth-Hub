  ...
  const calendarEvents = useMemo((): CalendarEvent[] => {
    ...
    tmpEvents.forEach((event) => {
      ...
      events.push({
        ...
        start: event.startAt ? parseISO(event.startAt.replace(' ', 'T')) : new Date(),
        type: 'tmp',
        game: event.game || 'ETS2',
        departure: `${event.departure?.city || 'Unknown'}${event.departure?.location ? `, ${event.departure.location}` : ''}`,
        arrival: `${event.arrive?.city || 'Unknown'}${event.arrive?.location ? `, ${event.arrive.location}` : ''}`,
        description: event.description,
        url: `https://truckersmp.com/events/${event.id}`,
        attendees: event.attendances?.confirmed || 0,
        server: event.server?.name || 'Simulator',
        banner: event.banner
      });
    });
    ...
    vtcEvents.forEach((event) => {
      events.push({
        id: `vtc-${event.id}`,
        title: event.title,
        start: parseISO((event.start_time || new Date().toISOString()).replace(' ', 'T')),
        type: 'vtc',
        game: event.game,
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2 pt-2">
                  <Button
                    ...
                    className={cn(
                      "flex-1 gap-2",
                      hasReminder(selectedEvent.id) && "bg-destructive hover:bg-destructive/90 text-destructive-foreground border-destructive"
                    )}
                  >
