...
export function PlayerStats() {
  ...
  const toggleUser = async (userId: string) => {
    ...
    setJobsLoading(true);
    try {
      const { data } = await supabase.from('job_logs').select('*').eq('user_id', userId).order('delivery_date', { ascending: false }).limit(5);
      setUserJobs(data || []);
    } catch(e) {
      console.error(e);
    } finally {
  ...
  const filteredStats = stats.filter(s => 
    ...
    (s.tmp_id && s.tmp_id.includes(search))
  );
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="relative w-full sm:w-auto">
          ...
          <Input 
            ...
            className="pl-9 glass-input w-full sm:w-64"
          />
        </div>
      </div>
                    ...
                    <td colSpan={7} className="px-4 py-6">
                      <div className="pl-14 space-y-3 relative">
                        ...
                        <h4 className="text-sm font-semibold flex items-center gap-2 text-foreground mb-4">
                          <Package size={16} className="text-primary"/> Recent Jobs (Last 5)
                        </h4>
              ...
              {filteredStats.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-muted-foreground">
                    No players found based on your search
                  </td>
                </tr>
              )}
            </tbody>
